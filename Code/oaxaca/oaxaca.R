...
.oaxaca.wrap <-
  ...
  function(formula, data, cluster, group.weights = NULL, R = 1000, reg.fun = lm, cl, ...) {
  ...
  run.oaxaca.bootstrap <- 
    #function(data, indices, formula.reg, formula.reg.collapse, formula.dummy, group.var, y, w.extra, reg.fun, R, ...) {
    function(data, cluster, indices, formula.reg, formula.reg.collapse, formula.dummy, group.var, y, w.extra, reg.fun, R, ...) {
     ...
     # clusters <- names(table(cluster))
      for (r in 1:R) {
        ...
        half <- n/2
        # Shuffle the clusters
        cluster_indices <- sample(length(cluster)/2, half, replace=T)

        # Select the first n cluster indices
        indices[[r]] <- c(cluster_indices[1:half], cluster_indices[1:half] + length(cluster)/2)
  ...
  if (!is.null(R)) { se <- run.oaxaca.bootstrap(cluster = cluster, formula.reg = formula.reg, formula.reg.collapse = formula.reg.collapse, formula.dummy = formula.dummy, data = data, group.var = group.var, y = y, w.extra = group.weights, reg.fun = reg.fun, R = R, ...) }
  else { se <- NULL }
